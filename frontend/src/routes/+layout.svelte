...
<footer class="container footer">
 ...
 <div class="foot-meta">{#if catalog.manifest}<span>{tr('Data updated','آخر تحديث')} {new Date(catalog.manifest.generated_at).toLocaleDateString(dateLocale(), { ...dateOptions(), day:'numeric',month:'short',year:'numeric' })}</span>{/if}<span>{tr('Prices are observations. Confirm price and availability with the seller.','الأسعار حسب آخر رصد. راجع السعر والتوفر مع المحل قبل الشراء.')}</span><nav class="foot-links" aria-label={tr('Project links','روابط المشروع')}><a href="https://walkthru.earth" target="_blank" rel="noopener">{tr('Walkthru.Earth','Walkthru.Earth')}</a><span aria-hidden="true">·</span><a href="https://github.com/walkthru-earth/borda/blob/main/LICENSE" target="_blank" rel="noopener">{tr('CC BY 4.0 license','رخصة CC BY 4.0')}</a></nav></div>
</footer>
<style>
