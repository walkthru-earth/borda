<script lang="ts">
	...
	import { trackProductView, trackSellerClick, trackDocumentClick } from '$lib/analytics';
	...
	let fromSellerText = $derived.by(() => { try { return detail?.extra_metadata ? JSON.parse(detail.extra_metadata)?.description_source === 'seller' : false; } catch { return false; } });
	...
	$effect(() => {
		const p = product;
		if (!p || loadingOffers || trackedProduct === p.id) return;
		trackedProduct = p.id;
		trackProductView({ product_id: p.id, name: p.canonical_name, group: p.group ?? null, brand: p.brand ?? null, sellers: latestOffers.length, lowest_price: bestOffer?.price ?? fallbackPrice ?? null, currency });
	});
...
<style>
	.crumbs { display:flex; align-items:center; flex-wrap:wrap; gap:10px; font-size:.9375rem; color:var(--muted); margin:8px 0 24px; }.crumbs a{display:flex;align-items:center;gap:7px}.crumbs a:hover,.doc-link:hover{color:var(--accent)}
	.panel{background:var(--surface,#fff);border:1px solid var(--line);border-radius:18px}.hero{display:grid;grid-template-columns:minmax(190px, .85fr) minmax(260px,1.5fr) minmax(220px,.9fr);gap:30px;padding:30px;margin-bottom:24px;align-items:start}.media{position:relative;aspect-ratio:1;display:grid;place-items:center;background:#f8fafb;border-radius:13px;color:#a4b5ba;overflow:hidden}.media img{width:100%;height:100%;object-fit:contain;mix-blend-mode:multiply;padding:24px 24px 45px}.category{position:absolute;bottom:14px;inset-inline-start:14px;background:white;border:1px solid var(--line);padding:5px 10px;border-radius:6px;font-size:.8125rem;color:var(--muted)}
	.eyebrow{text-transform:uppercase;letter-spacing:.12em;font-size:.8125rem;font-weight:700;color:var(--accent);margin-bottom:8px}.info h1{font-size:clamp(1.5rem,2.1vw,2rem);letter-spacing:-.035em;line-height:1.22;margin:0 0 14px;overflow-wrap:anywhere}.desc{font-size:1rem;line-height:1.8;color:var(--muted);margin:0 0 18px}.desc-source{font-size:.8125rem;color:var(--muted);margin:-12px 0 18px;opacity:.8}.product-meta{display:flex;flex-wrap:wrap;gap:12px;font-size:.875rem;color:var(--muted);margin-bottom:16px}.product-meta span{display:flex;align-items:center;gap:6px}.tags{display:flex;flex-wrap:wrap;gap:6px}.product-tag{background:#f4f7f8;color:#577078;border:1px solid #e8edef;padding:5px 9px;border-radius:6px;font-size:.875rem;overflow-wrap:anywhere}a.product-tag:hover{border-color:var(--accent);color:var(--accent)}.docs{margin-top:20px}.doc-link{display:inline-flex;align-items:center;gap:7px;font-size:.9375rem;font-weight:600;color:var(--accent)}
	.price-summary{background:#f1f8f6;padding:22px;border:1px solid #dcece7;border-radius:13px;display:flex;flex-direction:column;gap:9px}.price-summary .eyebrow{color:#55766e;margin:0}.headline-price{font-size:1.875rem;letter-spacing:-1px;line-height:1.25;font-variant-numeric:tabular-nums}.stock-note{display:flex;gap:5px;align-items:center;color:var(--muted);font-size:.875rem;line-height:1.5}.available{color:#087f63!important}.compare-button{display:flex;align-items:center;justify-content:center;gap:10px;margin:10px 0 1px;background:#087f73;color:white;padding:12px 10px;border-radius:8px;font-size:.9375rem;font-weight:650}.compare-button:hover{background:#06685e}.price-summary p{font-size:.8125rem;color:#647c75;line-height:1.7;margin:0}.last-seen{padding-top:12px;border-top:1px solid #dcece7;font-size:.8125rem;color:#647c75}
	.detail-columns{display:grid;grid-template-columns:1.05fr 1fr;gap:24px;align-items:start}.section-panel{padding:26px;margin-bottom:24px;min-width:0;scroll-margin-top:24px}.section-heading{display:flex;align-items:center;justify-content:space-between;gap:14px;margin-bottom:9px}.section-heading> :global(svg){color:#8aa49e}h2{font-size:1.1875rem;letter-spacing:-.025em;margin:0}.section-note{color:var(--muted);font-size:.875rem;line-height:1.6;margin:0 0 21px}.sellers{list-style:none;padding:0;margin:0;display:grid;gap:10px}.seller-row{display:grid;grid-template-columns:minmax(0,1fr) auto auto;gap:13px;align-items:center;padding:17px 13px;border:1px solid var(--line);border-radius:10px}.seller-row.best{border-color:#a8d5c9;background:#fbfefd}.seller-identity,.offer-info{display:flex;flex-direction:column;gap:6px}.seller-identity strong{font-size:.9375rem}.seller-identity span{font-size:.8125rem;color:var(--muted)}.offer-info{text-align:end}.offer-info strong{font-size:1rem;white-space:nowrap}.availability{font-size:.8125rem;color:var(--muted)}.visit{display:flex;align-items:center;gap:3px;font-size:.8125rem;font-weight:600;color:var(--accent);padding:7px;border-radius:6px;background:#edf6f3;white-space:nowrap}.visit:hover{background:#dcefe9}.best-label{grid-column:1/-1;font-size:.75rem;font-weight:600;color:#087f63;border-top:1px solid #e1f0eb;padding-top:10px}.quiet-empty{padding:45px 15px;text-align:center;background:#f8fafb;border-radius:10px;font-size:1rem;color:var(--muted)}
	.specs{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));margin:20px 0 0;gap:0 30px}.specs div{padding:13px 0;border-top:1px solid var(--line)}.specs dt{font-size:.875rem;color:var(--muted);margin-bottom:5px}.specs dd{font-size:1rem;font-weight:600;margin:0;overflow-wrap:anywhere}.links{list-style:none;display:flex;flex-wrap:wrap;gap:12px 24px;padding:10px 0;margin:0 0 14px}.seller-text{border-top:1px solid var(--line);padding:16px 0}.seller-text summary{cursor:pointer;font-size:.9375rem}.seller-text summary span{display:block;margin:7px 0 0;margin-inline-start:16px;color:var(--muted);font-size:.875rem}.seller-text p{white-space:pre-line;overflow-wrap:anywhere;font-size:.9375rem;line-height:1.9;color:var(--muted);margin:16px 0 0}.related{margin:14px 0 35px}.related .section-heading{margin-bottom:20px}.notice{display:flex;align-items:center;justify-content:space-between;gap:16px;border:1px solid #e7d9b1;background:#fffbef;border-radius:10px;padding:15px 18px;margin-bottom:24px;font-size:.9375rem}.empty{padding:60px 20px;text-align:center;display:flex;align-items:center;flex-direction:column;gap:15px}.empty h1{font-size:1.5rem;margin:0}.empty p{color:var(--muted);margin:0 0 5px}a:focus-visible,button:focus-visible,summary:focus-visible{outline:3px solid #74c5b7;outline-offset:4px}
	...
	/* Phones: the cover image would push the name and price below the fold – skip it. */
	@media(max-width:600px){.crumbs{font-size:.875rem;margin-bottom:18px}.hero{grid-template-columns:1fr;padding:19px;gap:22px}.media{display:none}.price-summary{display:flex;grid-column:auto;padding:19px}.info h1{font-size:1.5625rem}.section-panel{padding:19px;margin-bottom:18px}.detail-columns{gap:0}.seller-row{grid-template-columns:minmax(0,1fr) auto;gap:12px;padding:14px}.visit{grid-column:1/-1;justify-content:center;padding:9px}.specs{grid-template-columns:1fr 1fr;gap:0 18px}.related .section-heading{align-items:start}.related .doc-link{font-size:.8125rem}.notice{align-items:start;flex-direction:column}.headline-price{font-size:1.9375rem}}
	.original-name{font-size:.9375rem;color:var(--muted);margin:-6px 0 16px;text-align:start}.product-meta strong{unicode-bidi:isolate}.hero,.section-panel{min-width:0}:global([dir="rtl"]) .eyebrow,:global([dir="rtl"]) .info h1,:global([dir="rtl"]) h2,:global([dir="rtl"]) .headline-price{letter-spacing:0}:global([dir="rtl"]) .crumbs :global(svg){transform:scaleX(-1)}:global([dir="rtl"]) .eyebrow{font-size:.875rem}:global([dir="rtl"]) .info h1{line-height:1.55}:global([dir="rtl"]) .section-note{line-height:1.9}:global([dir="rtl"]) .seller-row{gap:10px}:global([dir="rtl"]) .visit{white-space:normal;text-align:center}
</style>
