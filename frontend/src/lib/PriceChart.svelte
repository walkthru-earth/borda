<script lang="ts">
	...
	let xTicks = $derived.by(() => {
		...
		const picks = unique.length <= 4 ? unique : [...new Set([unique[0], unique[Math.floor((unique.length - 1) / 3)], unique[Math.floor(2 * (unique.length - 1) / 3)], unique[unique.length - 1]])];
		const seen = new Set<string>(); // several runs on one day would print the same date twice, overlapping
		return picks.filter((t) => { const label = fmtDate(t); if (seen.has(label)) return false; seen.add(label); return true; });
	});
