...
| `catalog.parquet` | products: id, canonical_name / canonical_name_ar, **raw_names** (local/Arabic names), tags, description / description_ar (2-3 sentences), **specs** / specs_ar ("Key: value"), **mpn**, **datasheet_url**, brand, category, **group** (taxonomy), **image**, sellers, listings (JSON), **similar** (nearest neighbours), enriched | sorted by id, 2k-row groups, Bloom(id) |
...
history but are excluded from EGP comparisons. `latest_ts` records the product's newest valid
...
price. The displayed and sorted filtered price is the lowest eligible EGP listing price;
...
`uv run egmarket rebuild --no-embeddings` to enable the full filter contract.
